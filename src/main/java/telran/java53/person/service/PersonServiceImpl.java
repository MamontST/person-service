...
public class PersonServiceImpl implements PersonService, CommandLineRunner {
	...
	public PersonDto[] findPersonsByCity(String city) {
		return personRepository.findByAddressCityIgnoreCase(city).map(p -> mapToPersonDto(p))
	...
	public PersonDto[] findPersonsByName(String name) {
		return personRepository.findPersonsByNameIgnoreCase(name).map(p -> mapToPersonDto(p))
				.toArray(PersonDto[]::new);
	}
	...
	public PersonDto[] findPersonsBetweenAge(Integer minAge, Integer maxAge) {
		...
		return personRepository.findByBirthDateBetween(from, to).map(p -> mapToPersonDto(p))
				.toArray(PersonDto[]::new);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public EmployeeDto[] findEmployeesBySalary(int minSalary, int maxSalary) {
	...
	public void run(String... args) throws Exception {
		...
		if (personRepository.count() == 0) {
			...
			Child child = new Child(2000, "Moshe", LocalDate.of(2019, 7, 5), new Address("Ashkelon", "Bar kohva", 21),
					"Shalom");
